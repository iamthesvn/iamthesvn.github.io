...
coverImage:
  src: "https://pbs.twimg.com/media/G7Ggbuab0AAWril?format=png&name=900x900"
  alt: "TIL RUST #9 Image"
---
...
**Also, be careful with `>` versus `>>`.** 

A single `>` **overwrites** the file every time you run it. If you want to keep a log of previous runs, use `>>` to **append** the new output to the end of the file instead.
...
![cargo run with RUST_BACKTRACE=1](https://pbs.twimg.com/media/G7Ggbuab0AAWril?format=png&name=900x900)
...
![cargo run with RUST_BACKTRACE=full](https://pbs.twimg.com/media/G7GfiIEbsAADGq9?format=png&name=medium)
